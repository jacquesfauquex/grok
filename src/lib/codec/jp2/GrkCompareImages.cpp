...
static int parse_cmdline_cmp(int argc, char** argv, test_cmp_parameters* param)
{
	...
	try
	{
		...
		TCLAP::ValueArg<std::string> baseImageArg("b", "Base", "Base Image", true, "", "string",
												  cmd);
		TCLAP::ValueArg<std::string> testImageArg("t", "Test", "Test Image", true, "", "string",
												  cmd);
		TCLAP::ValueArg<uint32_t> numComponentsArg("n", "NumComponents", "Number of components",
												   true, 1, "uint32_t", cmd);
...
int GrkCompareImages::main(int argc, char** argv)
{
	...
	for(compno = 0; compno < imageBase->numcomps; compno++)
	{
		...
		else
		{
			...
			{
				spdlog::error("height mismatch [comp {}] ({} != {})", compno, baseComp->h,
							  testComp->h);
				goto cleanup;
			}
			...
			{
				spdlog::error("width mismatch [comp {}] ({} != {})", compno, baseComp->w,
							  testComp->w);
				goto cleanup;
			}
		}
