...
bool GrkImage::check_color(void)
{
	...
	if(clr->channel_definition)
	{
		...
		for(i = 0; i < n; i++)
		{
			...
			{
				Logger::logger_.error("Invalid component association %u  (>= %u).",
...
bool GrkImage::allocCompositeData(void)
{
	...
	for(uint32_t i = 0; i < numcomps; i++)
	{
		...
		{
			Logger::logger_.error("Output component %u has invalid dimensions %u x %u", i,
...
bool GrkImage::compositePlanar(const GrkImage* src)
{
	for(uint16_t compno = 0; compno < src->numcomps; compno++)
	{
		...
		{
			Logger::logger_.warn(
				"GrkImage::compositePlanar: null data for destination component %u", compno);
			continue;
		}
