...
bool CodeStreamCompress::init(grk_cparameters* parameters, GrkImage* image)
{
	...
	for(uint32_t i = 0; i < image->numcomps; ++i)
	{
		...
		if(comp->prec == 0)
		{
			Logger::logger_.error("Invalid component precision of 0 found while setting up JP2 compressor");
			return false;
		}
	}
...
bool CodeStreamCompress::writeTileParts(TileProcessor* tileProcessor)
{
	...
	{
		Logger::logger_.error("Number of tile parts %u for first POC exceeds maximum number of tile parts %u",
	...
	*numTilePartsForAllTiles = 0;
	for(uint16_t tileno = 0; tileno < numTiles; ++tileno)
	{
		...
		for(uint32_t pino = 0; pino < tcp->getNumProgressions(); ++pino)
		{
			...
			{
				Logger::logger_.error("Total number of tile parts %u for image exceeds JPEG 2000 maximum total "
						  "number of "
						  "tile parts %u",
